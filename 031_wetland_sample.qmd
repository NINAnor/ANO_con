...
I could also try and sample from a dummy dataset
...
dummy_sample <- spaceR::nested_balanced(
  samplingFrame = dummy,
  n_seq = seq(100, 10, -10),
  id_col = "id",
  stratum_col = "stratum",
...
one <- dummy |>
  group_by(stratum) |>
  summarise(mean_bal = mean(bal1)) |>
  add_column(sample = "all")


two <- dummy_sample$sample_100$ID |>
  as_tibble() |>
  rename(id = value) |>
  left_join(dummy, by = join_by(id)) |>
  group_by(stratum) |>
  summarise(mean_bal = mean(bal1)) |>
  add_column(sample = "100")

three <- dummy_sample$sample_50$ID |>
  as_tibble() |>
  rename(id = value) |>
  left_join(dummy, by = join_by(id)) |>
  group_by(stratum) |>
...
The means for each strata is not perfectly similar to the population mean. They are not way off either.
...
```{r}
#| include: false
saveRDS(prob_matrix, "data/prob_matrix.rds")
```

```{r}
#| eval: true
prob_matrix <- readRDS("data/prob_matrix.rds")
prob_matrix %>=%
  filter(!is.na(`50`)) %>=%
