...
## ANO våtmark 2025
Here we upload the dataframe with SSB-id's that make up the sampling frame for ANO våtmark (wetlands). The dataframe also contain auxillary variables used for balancing and stratifying the sampling. All the datasets and code (python) to create this dataframe if or will be uploaded to the database as well.
...
Units with snow and ice are kept, because they can become wetlands in a few decades.
...
Then we write data to the samplingframe_vaatmark_2025 table.
```{r}
...
## ANO Våtmark 2026
In 2026 we got a new map of bioclimatic sones and sections (and regions). In @sec-bcr we process this dataset and get the dominant regions for each SSB grid cell. Now we update the 2025 sampling frame with these regions, and also exclude the mid- and high alpine zones.

I cant just read in the 2025 sampling frame from the db, since there are some SSB cells missing from there because there was no biocliatic data available. So I redo the steps from above (from 2025).

```{r}
library(readr)
dat <- read_delim("P://412421_okologisk_tilstand_2024/Jan/ruter500m_Norge.csv", 
    delim = ";", escape_double = FALSE,  trim_ws = TRUE)
...
Read the stashed data with ssbid and the new bioclimatic regions and join it with `dat`.

```{r}
regions <- readRDS(here::here("data/avg_values.rds"))

regions <- regions |>
  mutate(ssbid = as.numeric(ssbid))

dat_j <- dat |>
  left_join(regions, by = join_by(ssbid))
...
# some cells more in regions, but I also excluded water above, so that makes sense

# Then I remove the mid and high alpines sones

dat_j <- dat_j |>
  filter(!is.na(region_ko)) |>
  filter(region_number != 99)

# Add fylkenavn, and geographic region

dat_f <- dat_j |> 
  mutate(fylk_navn = case_when(
    fylk2016 == "01" ~ "Østfold",
    fylk2016 == "02" ~ "Akershus",
    fylk2016 == "03" ~ "Oslo",
    fylk2016 == "04" ~ "Hedmark",
    fylk2016 == "05" ~ "Oppland",
    fylk2016 == "06" ~ "Buskerud",
    fylk2016 == "07" ~ "Vestfold",
    fylk2016 == "08" ~ "Telemark",
    fylk2016 == "09" ~ "Aust-Agder",
    fylk2016 == "10" ~ "Vest-Agder",
    fylk2016 == "11" ~ "Rogaland",
    fylk2016 == "12" ~ "Hordaland",
    fylk2016 == "14" ~ "Sogn og Fjordane",
    fylk2016 == "15" ~ "Møre og Romsdal",
    fylk2016 == "16" ~ "Sør-Trøndelag",
    fylk2016 == "17" ~ "Nord-Trøndelag",
    fylk2016 == "18" ~ "Nordland",
    fylk2016 == "19" ~ "Troms - Romsa",
    fylk2016 == "20" ~ "Finnmark - Finnmárku",
    .unmatched = "error"
  ),
  geographic_region = case_when(
    fylk_navn == "Østfold" ~ "Øst",
    fylk_navn == "Akershus" ~ "Øst",
    fylk_navn == "Oslo" ~ "Øst",
    fylk_navn == "Hedmark" ~ "Øst",
    fylk_navn == "Oppland" ~ "Øst",
    fylk_navn == "Buskerud" ~ "Øst",
    fylk_navn == "Vestfold" ~ "Øst",
    fylk_navn == "Telemark" ~ "Sør",
    fylk_navn == "Aust-Agder" ~ "Sør",
    fylk_navn == "Vest-Agder" ~ "Sør",
    fylk_navn == "Rogaland" ~ "Vest",
    fylk_navn == "Hordaland" ~ "Vest",
    fylk_navn == "Sogn og Fjordane" ~ "Vest",
    fylk_navn == "Møre og Romsdal" ~ "Midt",
    fylk_navn == "Sør-Trøndelag" ~ "Midt",
    fylk_navn == "Nord-Trøndelag" ~ "Midt",
    fylk_navn == "Nordland" ~ "Nord",
    fylk_navn == "Troms - Romsa" ~ "Nord",
    fylk_navn == "Finnmark - Finnmárku" ~ "Nord",
    .unmatched = "error"
  ))
```
...
### Define table properties
We will name the table *samplingframe_vaatmark_2026*.

```{r}
q1 <- "create table sampling_frames.samplingframe_vaatmark_2026 (
vaatmark_id character varying(50) primary key,
ssbid character varying(50),
ost integer,
nord integer,
...
komm2016 character varying(5),
fylk2016 character varying(5),
fylk_navn character varying(50),
geographic_region character varying(50),
r_ferskvann numeric(10,5),
r_hav numeric(10,5),
r_sno_isbre numeric(10,5),
r_bebygd_samf numeric(10,5),
mean_slope numeric(10,5),
ssbid_1000 numeric(15,0),
region_ko character varying(20),
region_number numeric(2,0),
abi integer,
CONSTRAINT fk_ssb_500
      FOREIGN KEY (ssbid)
        REFERENCES ssb_grids.ssb_500 (ssbid)
...
# indices makes the database work faster. It should be added to all tables that are looked up frequently
q2 <- "create index on sampling_frames.samplingframe_vaatmark_2026 using btree(ssbid);"
q3 <- "create index on sampling_frames.samplingframe_vaatmark_2026 using btree(vaatmark_id);"
```
...
Then we write data to the samplingframe_vaatmark_2026 table.
```{r}
write_sf(dat_f, dsn = con,
         layer = Id(schema = "sampling_frames", table = "samplingframe_vaatmark_2026"), 
         append = T)
```
