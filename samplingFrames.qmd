...
We start with `r nrow(dat)` sampling units, but 
we want to exclude sampling units with 100% water (ocean or freshwater).
There are 231083 units of marine waters and 
5207 with freshwater. 
...
```{r}
# nrow(dat[dat$rHav == 100,])
# nrow(dat[dat$rFerskvann == 100,])

 dat2 <- dat |>
  dplyr::filter(rHav < 100 & rFerskvann <100)

# nrow(dat)-nrow(dat2) # 236290. Correct
```
