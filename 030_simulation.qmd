...
for (n in seq(80,20, -20)) {
  ...
  Xspread <- tibble(
    ...
    mutate(ID = row_number(),
           E = case_when(
             ...
             .default = 3), # test with 3 to see if we get a bias
           E = case_when(
  ...
  prob = temp$pi,
  Xspread = Xspread, 
  # OBS, I'm not sure the spreading is working perfectly when using nested samples. 
  # The algorithm might try to spread the population units, but 
  # we need it to spread the sub-population (previous sample) units. 
  # Setting values in Xspread to NA's do not work (it makes it crash)
  # Setting values to something very high or low migth work,
  # as long as it doesn't 'push' away any of the remaining units, 
  # making them less likely to become selected and thus introduces a spatial bias.
  Xbal = X4,
  integerStrat = mypop$stratum)
