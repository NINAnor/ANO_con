...
mypop <- mypop |>
  ...
  # add dummy auxiliary variables
         aux1 = seq(1,5, length.out = n()),
         aux2 = rpois(n = n(), lambda = 5)) |>
  ungroup() |>
  # round values
...
# Create model matrix by expanding the strat column, explicitly assigning both prencenses and abcenses to each row.
...
```{r fig-dummyData}
ggplot(data = mypop) +
  geom_tile(mapping = aes(x = Easting, y = Northing, fill = factor(stratum)), width = 1, height = 1, linewidth = 0.5, colour = "white") +
  scale_fill_viridis_d(name = "Stratum") +
  coord_fixed()
```


Then I use the cube method in to select a sample balanced on the strata.
Here I use the `sampling` package.
```{r fig-bal1}
#| fig-cap: 'Sample balanced on a categorical variable (strata).'
set.seed(314)
...
In @fig-bal1, _n_= 5 for each strata, but this is not explicitly defined.
The total _n_ across all strata is defined through the sum of the inclusion probability vector, which is 20.
