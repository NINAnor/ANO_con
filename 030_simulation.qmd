...
tableOfMeans
# Later, drop the first four rows and pivot_longer for SRSS, and wider or 'variable', so that I can colour the columnn
...
It appears from comparing @fig-ssrs with @fig-ssrs2 that the latter has a better overlap with the population distribution, although with n = 10 this is subject to a lot of chance still.
...
set.seed(3143)
sample_ind4 <- sampling::samplecube(X = X2, pik = mypop$pi_pps, comment = TRUE, method = 1)
mysample4 <- mypop[sample_ind4 > (1 - eps), ]

ht_results <- lapply(vars, function(v) {
  y <- mysample4[[v]]
  res <- mase::horvitzThompson(y = y, pi = mysample4$pi_pps, var_est = TRUE)
  data.frame(
...
in out example, using pps strongly disfavoured population units below A Northing of 30 (@fig-pps). 
...
The samples do look like they are well-spread lready, and not clumped.
This might get differnt if n was bigger. @fig-higherN does show two and three eighbouring cells getting selected.

```{r fig-higherN}
#| eval: true
#| fig-cap: 'Balanced sample with n = 40'
mypop <- mypop |>
  mutate(pi_h2 = rep(40, 4) / N_h * area)
...
We can spread the samples geographically by balancing on the coordinates, but there are better ways, such as the local pivotal method. 
...
Comparing @fig-lcube with @fig-higherN there are noticeably more clumping in the latter. @tbl-relDiff shows us that the relative difference between the Horvitz-Thompson estimators for the population mean is very similar across all our samples. 
Perhaps most surprising is that SSRS was so good with aux2, an expoential variable. But it could also be that n=5 is too little for getting stable and robust measures of deviation. A better validation would be to boostrap the process and look at the variation in means then. 
...
ht_results <- lapply(vars, function(v) {
  y <- mysample6[[v]]
  res <- mase::horvitzThompson(y = y, pi = mysample6$pi_h3, var_est = TRUE)
  data.frame(
