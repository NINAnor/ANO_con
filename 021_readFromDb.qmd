...
> This feature is not working due to problems defining foreign keys in the database

But now we import the two tables separately and join them. 
<!-- 
...
wetlandFrame <- dplyr::tbl(
  con,
  dbplyr::in_schema("sampling_frames", "samplingframe_vaatmark_2026")
) |>
  collect()


wetland2026 <- dplyr::tbl(
  con,
  dbplyr::in_schema("samples", "vaatmark_2026")
) |>
  collect()

# I want to get the n70 sample, with geometries
wetland2026_70 <- wetland2026 |>
  filter(!is.na(`70`)) |>
  select(ano_vaatmark_2026_flateid, ssbid) |>
  left_join(
    wetlandFrame |> select(ssbid, komm2016, fylk2016, geographic_region),
    by = join_by(ssbid)
  ) |>
  left_join(
    ssb500 |> select(-geom_wkb),
    by = join_by(ssbid)
  ) |>
  st_as_sf()
```
...
tm_shape(wetland2026_70) +
  tm_polygons(col = "geographic_region")
```

Then I export this as a geopackage
```{r}
path <- "R:/Kladd/ANOvaatmark"
st_write(wetland2026_70, paste0(path, "/vaatmark_2026_n70.gpkg"), append = FALSE)
```
